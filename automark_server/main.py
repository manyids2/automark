...
def check_answer(username='', assignment='', ipd=-1, answer=''):
    ...
    answer_is_correct = np.allclose(x_true, x_answer, atol=1e-5) and x_true.shape == x_answer.shape
...
@app.route('/_admin/grades/csv')
def progress_sort_done():
    assignment_names = Global.data_dict.keys()
    progress = utils.get_users_progress()
    user_dicts = []
    si = io.StringIO()
    cw = csv.DictWriter(si, ['Student ID', 'completed'])
    cw.writeheader()
    for user in utils.get_users_list():
        completed = 0
        for assignment in assignment_names:
            if progress[user].get(assignment, False):
                completed += 1
        dict_ = {
            'Student ID': user,
            'completed': round(completed / len(assignment_names) * 100)
        }
        cw.writerow(dict_)

    output = make_response(si.getvalue())
    output.headers["Content-Disposition"] = "attachment; filename=grades.csv"
